...
def get_accuracy(model: torch.nn.Module,
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            ...
            output,outputs_stu = model([img.to(device) for img in imgs]) if isinstance(imgs, list) else model(imgs.to(device))
            predictions = output.argmax(1)

            outputs_stu = torch.nn.functional.softmax(outputs_stu, dim=1)
            max_probs.append(outputs_stu.max(1)[0].cpu().numpy())
    ...
    # 画出最大概率的分布曲线并保存 名字：cotta_domain_name_平均值
    if len(max_probs) > 0:
        max_probs = np.concatenate(max_probs)
        avg_max_prob = max_probs.mean()
        import matplotlib.pyplot as plt
        plt.hist(max_probs, bins=100, range=(0, 1))
        plt.title(f"Max Probabilities of {domain_name}")
        plt.xlabel("Max Probability")
        plt.ylabel("Count")
        plt.savefig(f"cotta_{domain_name}_{avg_max_prob:.6f}.png")
        plt.close()
